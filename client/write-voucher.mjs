...
import { FileText, Search, MapPin, Fuel, CreditCard, Wallet, Pencil, Trash2, Printer, Check, X, AlertTriangle, Plus, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
<div class="total"><span>GROSS TOTAL</span><span>Rs.\${(v.total||calc.total).toLocaleString()}</span></div>
<div class="div"></div>
      ...
      <motion.div initial={{opacity:0,scale:0.94,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0}}
        ...
        <div style={{ padding:'20px 22px',display:'flex',flexDirection:'column',gap:'12px' }}>
          <div className="fg fg-2">
            ...
            <div className="field"><label>Truck No.</label><input className="fi" type="text" value={form.truckNo} onChange={e=>S('truckNo',e.target.value)}/></div>
        ...
        <div style={{ display:'flex',gap:'10px',padding:'14px 22px',borderTop:'1px solid var(--border)',justifyContent:'flex-end' }}>
          ...
          <button className="btn btn-p" onClick={handleSave} disabled={saving}>{saving?'Saving...':<><Check size={14}/> Save Changes</>}</button>
        </div>
      </motion.div>
    </div>
  );
...
export default function VoucherModule() {
  ...
  const totals = useMemo(() => ({
    ...
    total:  filtered.reduce((s, v) => s + (parseFloat(v.total) ||0), 0),
  }), [filtered]);
...
writeFileSync('B:/VGTC Managemet/client/src/modules/VoucherModule.jsx', code, 'utf8');
console.log('VoucherModule.jsx written:', code.length, 'chars');
