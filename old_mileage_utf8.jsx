...
const DIESEL_PRICE_PER_LITRE = 90; // estimated Γé╣ per litre for mileage calculation

function getMileageColor(kmPerL) {
    if (!kmPerL || kmPerL <= 0) return 'var(--text-muted)';
    if (kmPerL >= 4) return '#10b981'; // green ΓÇö good
    if (kmPerL >= 2.5) return '#f59e0b'; // yellow ΓÇö average
    return '#f43f5e'; // red ΓÇö poor
}
...
/* ΓöÇΓöÇ Per-Vehicle Detail View ΓöÇΓöÇ */
            ...
            {mileageTrips.length > 0 ? (
                <>
                    <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
                        ...
                        <StatCard icon={Fuel} label="Total Diesel Advance" value={`Γé╣${stats.totalDieselRs.toLocaleString()}`} sub={`Γëê ${(stats.totalDieselRs / DIESEL_PRICE_PER_LITRE).toFixed(0)} litres @ Γé╣${DIESEL_PRICE_PER_LITRE}/L`} color="#3b82f6" />
                        <StatCard
                            icon={Gauge}
                            label="Avg Mileage"
                            value={`${stats.avgKmPerL} km/L`}
                            sub={parseFloat(stats.avgKmPerL) >= 4 ? 'Γ£à Good mileage' : parseFloat(stats.avgKmPerL) >= 2.5 ? 'ΓÜá∩╕Å Average mileage' : '≡ƒö┤ Poor mileage'}
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px' }}>
                        <thead>
                            <tr style={{ background: 'var(--bg-th)' }}>
                                {['LR #', 'Date', 'Destination', 'Start KM', 'End KM', 'Distance', 'Diesel Adv (Γé╣)', 'Pump', 'Mileage'].map(h => (
                            ...
                            {trips.length === 0 ? (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', color: 'var(--text-sub)' }}>{t.destination || 'ΓÇö'}</td>
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', color: 'var(--text-sub)', fontFamily: 'monospace' }}>{t.startKm || 'ΓÇö'}</td>
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', color: 'var(--text-sub)', fontFamily: 'monospace' }}>{t.endKm || 'ΓÇö'}</td>
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', fontWeight: 800, color: dist ? '#6366f1' : 'var(--text-muted)' }}>
                                            {dist != null ? `${dist.toFixed(0)} km` : 'ΓÇö'}
                                        </td>
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', color: 'var(--text-sub)' }}>
                                            {dieselRs > 0 ? `Γé╣${dieselRs.toLocaleString()}` : t.isFullTank ? 'FULL TANK' : 'ΓÇö'}
                                        </td>
                                        <td style={{ padding: '9px 13px', borderBottom: '1px solid var(--border-row)', color: 'var(--text-sub)', fontSize: '11px' }}>{t.pump || 'ΓÇö'}</td>
...
/* ΓöÇΓöÇ Vehicle Selector Grid ΓöÇΓöÇ */
        ...
        <div className="page-container">
            ...
            <div className="page-hd">
                <div>
                    ...
                    <p>Per-vehicle km/litre analytics ΓÇö Vikas Goods Transport (VGTC)</p>
                </div>
            </div>
            ...
            <div style={{ marginBottom: '18px', padding: '12px 16px', background: 'rgba(99,102,241,0.07)', border: '1px solid rgba(99,102,241,0.18)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12px', color: 'var(--text-sub)', fontWeight: 600 }}>
                ...
                Mileage is tracked from <strong>Dump Vouchers</strong>. Fill Start KM &amp; End KM when creating a voucher to build this history. Mileage estimates use Γé╣{DIESEL_PRICE_PER_LITRE}/litre as the diesel price.
            </div>
