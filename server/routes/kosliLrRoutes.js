...
router.post('/', async (req, res) => {
    try {
        ...
        backupLoadingReceipt({ ...bodyWithMeta, ...result }, { plant: 'Kosli', brand: 'kosli' });
    ...
    try {
        ...
        if (ids && ids.length > 0) {
            try {
                ...
                console.warn('[Invoice] Kosli LR update failed (non-fatal):', dbErr.message);
            }
        }
        ...
        if (await driveService.isAuthorized().catch(() => false)) {
            (async () => {
                try {
                    ...
                    console.log(`[Backup-Hook] Kosli Invoice backed up: ${fileName} in ${segments.join('/')}`);
                } catch (e) {
                    console.error('[Backup-Hook] Kosli Invoice backup FAILED:', e.message);
                }
            })();
        }
