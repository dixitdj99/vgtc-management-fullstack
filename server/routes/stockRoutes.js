...
router.get('/materials/list', async (req, res) => {
    try { res.json(await svc.getMaterialsList('materials')); }
    catch (e) { res.status(500).json({ error: e.message }); }
});
router.post('/materials', async (req, res) => {
    try { res.status(201).json(await svc.addMaterial(req.body.name, 'materials')); }
    catch (e) { res.status(400).json({ error: e.message }); }
});
router.delete('/materials/:id', async (req, res) => {
    try { await svc.deleteMaterial(req.params.id, 'materials'); res.json({ ok: true }); }
    catch (e) { res.status(404).json({ error: e.message }); }
});
...
router.post('/sync-lr', async (req, res) => {
    try {
        ...
        await svc.syncLRWithChallans(oldChallanNos, newChallanNos, material, quantity);
        res.json({ ok: true });
    } catch (e) { res.status(400).json({ error: e.message }); }
});
