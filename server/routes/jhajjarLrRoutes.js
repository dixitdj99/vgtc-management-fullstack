...
router.post('/', async (req, res) => {
    try {
        ...
        if (await driveService.isAuthorized()) {
            ...
            await sheetsService.upsertLrRow({ ...bodyWithMeta, ...result }, 'jksuper').catch(()=>{});
        }

        const { backupLoadingReceipt } = require('../utils/realtimeBackup');
        backupLoadingReceipt({ ...bodyWithMeta, ...result }, { brand: 'jksuper', plant: 'Jhajjar' });

        res.status(201).json(result);

        // WhatsApp Notification — fire and forget, never blocks response
        dispatchLrNotification({ ...bodyWithMeta, ...result }, req);
    ...
    try {
        ...
        if (ids && ids.length > 0) {
            try {
                ...
                console.warn('[Invoice] Jhajjar LR update failed (non-fatal):', dbErr.message);
            }
        }
        ...
        if (await driveService.isAuthorized().catch(() => false)) {
            (async () => {
                try {
                    ...
                    console.log(`[Backup-Hook] Jhajjar Invoice backed up: ${fileName} in ${segments.join('/')}`);
                } catch (e) {
                    console.error('[Backup-Hook] Jhajjar Invoice backup FAILED:', e.message);
                }
            })();
        }
